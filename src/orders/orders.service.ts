...
export class OrdersService {
  ...
  findAll(query: QueryOrdersDto) {
    ...
    return this.prisma.order.findMany({ where, include: { branch: true, sale: true }, orderBy: { dueDate: 'asc' } });
  }

  async findOne(id: number) {
    const order = await this.prisma.order.findUnique({ where: { id }, include: { branch: true, sale: true } });
  ...
  async convertToSale(id: number, dto: ConvertOrderToSaleDto) {
    const order = await this.findOne(id);
